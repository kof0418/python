...
class Database:
                ...
                try:
                    if self.user != "" and self.passwd != "" and self.name != "":
                        self.cnx = mysql.connector.connect(
                            host="localhost", user=self.user, password=self.passwd, database=self.name)
    ...
    def get_registrable(self):
        ...
        self.execute(
            "SELECT params FROM {0}extensions WHERE name = 'com_users'".format(self.prefix))
        ...
        # For Joomla 1.5:
        self.execute(
            "SELECT params FROM {0}components WHERE `option` = 'com_users'".format(self.prefix))
        if self.is_success():
            params = self.cursor.fetchone()
    ...
    def get_contacts(self):
        ...
        self.execute(
            "SELECT * FROM {0}contact_details WHERE published = '1' AND params LIKE '%show_email_copy%'".format(self.prefix))
            ...
            if fn == "configuration.php":
                ...
                if db.is_joomla():
                    if db.is_available():
                        ...
                        db.close()
                        log.add("{0},{1},{2},{3},{4}".format(
                            full_path, db.name, users, registrable, contacts))
                    else:
                        log.add("{0},{1},Failed to connect".format(
                            full_path, db.name))
