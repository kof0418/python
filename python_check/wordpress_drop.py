...
def get_tries(ip, path):
    ...
    grep_result = get_cmd_output(
        "nice -n 19 grep -Ehr \"{0}.*{1}.*POST.*{2}.* 200 \" {3}|wc -l".format(ip, today, path, APACHE_LOG_PATH))
            ...
            if allowed_tries != 0 and tries >= allowed_tries and get_ip_blacklisted(ip):
                if not quiet:
                    print("{0}: {1}: {2} ({3})".format(
                        ip, country, path, tries))

                if drop == True:
                    if CSF_MODE == False:
                        run_cmd(
                            "/usr/sbin/ipset add {0} {1} -exist".format(IPSET, ip))
                    elif CSF_MODE == True:
                        run_cmd(
                            "/usr/sbin/csf -d {0} \" {1} from {2}\"".format(ip, path, country))

                    dropped_ip.append(ip)
            else:
                if not quiet:
                    print("{0}: {1}: {2} ({3}) passed".format(
                        ip, country, path, tries))
