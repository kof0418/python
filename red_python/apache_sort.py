...
def apache_sort():
    if len(sys.argv) < 2 or sys.argv[1] not in OPTION_LIST:
        print("Usage: {0} <{1}> <(optional) keyword>".format(sys.argv[0], ", ".join(OPTION_LIST)))
        return False

    if len(sys.argv) >= 3:
        status = run_cmd("/usr/sbin/apachectl fullstatus|grep {0}".format(sys.argv[2]))
    else:
    ...
    for count, elem in sorted(((data_list.count(i), i) for i in set(data_list))):
        if sys.argv[1] == "client-detailed":
            print("{0}: {1} ({2} {3})".format(elem, count, get_ip_country(elem), get_ip_domain(elem)))
        else:
