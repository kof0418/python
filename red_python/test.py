...
def check_memory():
    ...
    if mem_total < 1048576:
        msg = "{0}% - {1} / {2} MB used. Free memory: {3} MB, min free memory: {4} MB.".format(percentage, int(used_mem / 1024), int(mem_total / 1024), int(mem_free / 1024), int(min_free_kbytes / 1024))
    else:
        msg = "{0}% - {1} / {2} GB used. Free memory: {3} MB, min free memory: {4} MB.".format(percentage, int(used_mem / 1024 / 1024), int(mem_total / 1024 / 1024), int(mem_free / 1024), int(min_free_kbytes / 1024))
